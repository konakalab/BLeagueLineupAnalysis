...
with tab1:
    ...
    # --- リーグ全体か特定チームかで判定ロジックを分岐 ---
    if target_team_id is None:
        # リーグ全体選択時：すべて「リーグ全体」として扱い、不透明度を一定にする
        df_all_p['is_selected'] = True
        df_all_p['DisplayGroup'] = "リーグ全体"
        color_map = {"リーグ全体": "#636EFA"} # リーグ全体用の色（青系）
        opacity_map = {True: 0.6}
    else:
        # 特定チーム選択時：既存のロジック
        ...
        opacity_map = {True: 1.0, False: 0.3}
    
    # プレイ合計数とバブルサイズ
    df_all_p['TotalApps'] = df_all_p['OFFApps'] + df_all_p['DEFApps']
    df_all_p['MarkerSize'] = np.sqrt(df_all_p['TotalApps'] + 1)
    
    # ラベル表示（特定チーム選択時のみ背番号を表示）
    df_all_p['Label'] = df_all_p.apply(
        lambda r: str(int(r['PlayerNo'])) if target_team_id and r['is_selected'] and r['PlayerNo'] != 0 else "", axis=1
    )
    
    # 描画順：強調したい方を後に（上に）持ってくる
    ...
    fig_p = px.scatter(
        ...
        opacity=df_all_p['is_selected'].map(opacity_map)
    )

    # (以下、fig_p.update_layout 等の設定は既存のまま)
    # ... [前回の update_layout 設定を適用] ...
