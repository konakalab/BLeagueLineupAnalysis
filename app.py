...
def load_all_data():
    ...
    # 列名の前後にある空白のみを削除（大文字・小文字は維持）
    for df in [df_t, df_p, df_l]:
        df.columns = [str(c).strip() for c in df.columns]
        
        # 数値型へ変換（元のCSVの列名に準拠）
        # ※ table_team.csv には 'Order' がある前提
        numeric_cols = [
