...
with tab1:
    ...
    with st.expander("💡 独自指標「 **kWAR** 」について"):
        ...
        * kWARの定義では，基準となるレベルの選手(replacement level)を「リーグの平均的な選手のみのチームと対戦したときに予測される勝率が10％(現在のレギュラーシーズンの方式では6勝54敗)」としています．
        * 基準選手のみで構成されている2チームのうち，1名のみ着目する選手と交代し，実際の出場時間に対応した時間だけプレイしたと仮定したとき，予測勝率が0.5からどれだけ変化するかを計算します．(1試合当たりの予測勝率の増加量)
        　* 上記の「1試合当たりの予測勝率の増加量」に，実際に何試合相当プレイしているのかを掛けたものを **kWAR** と定義します．
    ...
    # 表示する列のリストを定義
    cols_base = ['PlayerNo', 'PlayerNameJ', '公式サイト','kWAR', '総合評価', '実得点', '得点期待値', 'TotalApps', '貢献量',  'HensatiOFF', 'HensatiDEF']
    if is_league_mode:
...
with tab3:
    ...
    * **貢献量**: プレイ単位の**小さな勝利** の確率を推計し，その攻守にわたる合計を算出しています．基準を「平均的なチームと対戦したときに予測される試合全体の勝率が10％程度(レギュラーシーズン6勝54敗)であるランナップ」とし，そのうちの1名が当該選手と交代したときに増減するプレイの勝率を計算します．合計値は(プレイ数)x(プレイ勝率の増加値)です．
    * **kWAR** (**W**in **A**bove **R**eplacement level defined by **k**onakalab)はkonakalabが独自に算出している選手の評価指標です．貢献量を勝利数に換算した値です．
